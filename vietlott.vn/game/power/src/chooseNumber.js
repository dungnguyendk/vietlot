var ChooseNumber = (function () {
    ...
    ChooseNumber.prototype.create = function () {
        ...
        var logo = this.game.add.image(5, this.game.world.centerY / 2, Resources.logo_game.name);
        ...
        // descript text
        var txtDes1 = this.game.add.text(795, 130, '- Giá vé 10.000đ/\nlần tham gia dự thưởng.', {
            font: "14px Arial", fill: "#000000", wordWrap: true,
            wordWrapWidth: 400, align: "left", boundsAlignH: 'center', boundsAlignV: 'center'
        });
        var txtDes2 = this.game.add.text(795, 190, '- Giờ quay số mở thưởng: \n18h10 ngày Thứ 3, Thứ 5 \nvà Thứ 7 hàng tuần.', {
            font: "14px Arial", fill: "#000000", wordWrap: true,
            wordWrapWidth: 400, align: "left", boundsAlignH: 'center', boundsAlignV: 'center'
        });
        var txtDes3 = this.game.add.text(795, 300, '  Các số đã chọn:', {
            font: "14px Arial", fill: "#000000", wordWrap: true,
            wordWrapWidth: 400, align: "left", boundsAlignH: 'center', boundsAlignV: 'center'
        });
        // Buy Button
        var btnBuy = this.game.add.button(870, 560, Resources.btn_green.name, function () {
            _this.buyProcess();
        });
        ...
        //}, this);
        // Rule Button
        var btnRule = this.game.add.button(823, 113, Resources.ico_question.name, function () {
            OpenRule(2);
        });
        btnRule.anchor.set(0.5, 0.5);
        var txtRule = this.game.add.text(btnRule.x + 22, btnRule.y + 2, 'Luật Chơi', {
        ...
        this.txtTicket = this.game.add.text(795, 320, 'A:\nB:\nC:\nD:\nE:', {
            font: "14px Arial", fill: "#000000", wordWrap: true,
            wordWrapWidth: 400, align: "left", boundsAlignH: 'center', boundsAlignV: 'center'
        });
    ...
    ChooseNumber.prototype.drawTicket = function () {
        ...
        var ticketW = 125;
        var ticketSpace = 3;
        var lstTickerPositionX = [140, 140 + (ticketW + ticketSpace) * 1,
            140 + (ticketW + ticketSpace) * 2,
            140 + (ticketW + ticketSpace) * 3,
            140 + (ticketW + ticketSpace) * 4];
        var ticketPositionY = 96;
        for (var i = 0; i < 5; i++) {
            var ticketBg = this.game.add.image(lstTickerPositionX[i], ticketPositionY, Resources.panel_655.name);
            ...
            var txtTC = this.game.add.text(ticketBg.x + 44, ticketBg.y + 502, '     ', {
            ...
            var txtCancel = this.game.add.text(ticketBg.x + 104, ticketBg.y + 502, '     ', {
                font: "18px Arial", fill: "#000000", wordWrap: true,
                wordWrapWidth: 400, align: "center", boundsAlignH: 'center', boundsAlignV: 'middle'
            });
            ...
            for (var j = 0; j < 55; j++) {
                ...
                row = (j) % 11;
                collum = Math.floor((j) / 11);
                var bar = this.game.add.graphics(ticketBg.x + 19 + numberSpaceH * collum, ticketBg.y + 58.5 + numberSpaceV * row);
                var txtNumber = this.game.add.text(ticketBg.x + 19 + numberSpaceH * collum, ticketBg.y + 58.5 + numberSpaceV * row, ' ' + (j + 1) + ' ', {
    ...
    ChooseNumber.prototype.drawBao = function () {
        this._lstBaoObject = [];
        var ticketBg = this.game.add.image(95, 96, Resources.bg_bao.name);
        var numberSpaceV = 40;
        for (var i = 0; i < 11; i++) {
            var bao = 0;
            if (i == 0)
    ...
    ChooseNumber.prototype.generateRandomNumber = function (quantity) {
        ...
        while (true) {
            ...
            var newNumber = Math.floor(rand * 55) + 1;
            var isContain = false;
